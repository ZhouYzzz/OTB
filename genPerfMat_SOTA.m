...
for idxSeq=1:length(seqs)
    ...
    rect_anno = s.groundtruth_rect; %dlmread([pathAnno s.name '.txt']);
    numSeg = 20;
	...
	fprintf(sprintf('%12s\t%12s\t%s\n', s.name, t.name, num2str(mean(aveSuccessRatePlot(idxTrk,idxSeq,:)))));
        % fprintf([s.name, ',', t.name,',',num2str(mean(aveSuccessRatePlot(idxTrk,idxSeq,:))),'\n']);
    end
end
%
