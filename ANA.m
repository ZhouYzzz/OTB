function ANA(seqs, features)
%% Perform ANA Evaluation
evalType='ANA';
...
for idxSeq = 1:numSeq
    ...
    for idxFtr = 1:numFtr
        ...
        for idx=1:length(subSeqs)
            disp([num2str(idxFtr) '_' t.name ', ' num2str(idxSeq) '_' s.name ': ' num2str(idx) '/' num2str(length(subSeqs))])       
